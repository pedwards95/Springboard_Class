...
router.get("/", async function(req, res, next) {
    try{ 
        let results = await db.query(`SELECT id, name, type FROM users`);
...
router.get('/:id', async (req,res,next) => {
    try {
        const {id} = req.params;
        const results = await db.query('SELECT name, type FROM users WHERE id = $1',[id])
        const resultsM = await db.query('SELECT id, msg FROM messages WHERE user_id = $1',[id])
        if(results.rows.length === 0) {
            throw new ExpressError(`Cant find user with id of ${id}`,404)
        }
        const user = results.rows[0];
        user.messages=resultsM.rows;
